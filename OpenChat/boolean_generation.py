...
import random
from model_is import final_boolean_statement
    ...
    for index,item in enumerate(Text):
        ...
        if len(item)>100:
            ...
            messages = [
                    {
                        "role": "system",
                        "content": "You are best in generating Yes/No Questions. You have to generate both yes and no Questions. For no Questions strictly change named entity or phrase of the Questions. Generate response as per demanded JSON format."
                    },
                    {"role": "user", "content":"Please summarize the provided text and generate exclusively Boolean questions that strictly evaluate to Yes. The text is as follows: "+" "+item+'''Please Give me output Only in JSON format:[{"question": string,"answer": 'Yes'}]'''
            }]
            ...
            messages = [
                    {
                        "role": "system",
                        "content": "You are best in generating false Questions by changing named entity or phrase of the true Questions.Generate response as per demanded JSON format."
                    },
                    {"role": "user", "content": llm_output + '''From above text, make Questions false by changing any named entity or phrase.For making false Questions, please always keep one thing in mind that new named entity should be semantically near to old one.'''
             }]
            prompt = pipe.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
            outputs = pipe(prompt, max_new_tokens=786, do_sample=True, temperature=0.1, top_p=0.9)
